 ...
 * 1. match_time > now (strictly in the future)
 * 2. status strictly in ('SCHEDULED', 'TIMED') - never 'FINISHED' or 'IN_PLAY'
 */
...
function extractCandidateLegs(fixture: Fixture): LegSelection[] {
  ...
  const homeForm = fixture.homeTeam?.form;
  const awayForm = fixture.awayTeam?.form;
  ...
  if (qa.trueProbabilities.bttsYes >= 0.55 && odds?.btts_odds?.btts_yes) {
    legs.push({
      fixtureId: fixture.id,
      league: fixture.league,
      homeTeam,
